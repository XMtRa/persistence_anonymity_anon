...
Also note that we are here simulating data which we then analyze. The data simulation is somewhat rudimentary. 

- First, In reality, the data are nested: in groups (some groups are more active than others) and topics (some topics elicit more comments than others). Because we are lacking information on the covariance structure, we did not implement the data's nested structure. In conclusion, we also did not analyze the data using mixed effects models, which account for hierachical structure. 
- Second, opposed to what we model here, our outcome variable is likely not normally distributed. As a result, if this turns out to be the case we will also analyze the data using Bayesian models that can model the outcome variable with for example a poisson or gamma distribution, also taking into account zero-inflation. 
...
sim_d <- function(d_frame, 
  ...
  for(i in 1 : repetition_n) {
    d_frame[d_frame$persistence == 0 & d_frame$anonymity == 0 & d_frame$repetition == i, ]$expressions <- 
      rnorm(groupsize_n, effects["pers0_anon_0_m"], sd)
    d_frame[d_frame$persistence == 0 & d_frame$anonymity == 1 & d_frame$repetition == i, ]$expressions <- 
      rnorm(groupsize_n, effects["pers0_anon_1_m"], sd)
    d_frame[d_frame$persistence == 1 & d_frame$anonymity == 0 & d_frame$repetition == i, ]$expressions <- 
      rnorm(groupsize_n, effects["pers1_anon_0_m"], sd)
    d_frame[d_frame$persistence == 1 & d_frame$anonymity == 1 & d_frame$repetition == i, ]$expressions <- 
...
analyze_d <- function(object, approach, ...) {
  ...
  if(approach == "bayes_factor") {
    ...
    bf <- BayesFactor::regressionBF(expressions ~ persistence + anonymity, object, progress = F)
...
write.csv(d, "data_simulated.csv") # save data.
```
...
```{r}
bf <- BayesFactor::regressionBF(expressions ~ persistence + anonymity, d)
summary(bf)
```
